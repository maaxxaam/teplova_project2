...
ax.plot([i for i in range(101)], R)
ax.set_title('Радиальная функция при n = '+str(n)+' и l = '+str(l))
ax.set_ylabel('R(rho)')
ax.set_xlabel('r*10**(-13)')
fig.tight_layout()
fig, ay = plt.subplots(figsize=(6, 6))
ay.plot([i for i in range(101)], [i**2 for i in R])
ay.set_title('Квадрат радиальной функция при n = '+str(n)+' и l = '+str(l))
ay.set_ylabel('R(rho)')
ay.set_xlabel('r*10**(-13)')
fig.tight_layout()
fig, az = plt.subplots(figsize=(6, 6))
az.plot([i for i in range(101)], D)
az.set_title('Вероятность обнаружения электрона при n = ' + str(n) + ' и l = ' + str(l))
az.set_ylabel('D(rho)')
az.set_xlabel('r*10**(-13)')
